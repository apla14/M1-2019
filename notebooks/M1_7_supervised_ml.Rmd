...
```{r}
pred.lm <- predict(fit.glmnet, newdata = x_test) 
...
![](mhttps://www.dropbox.com/s/dwl89havzpl1qla/regtree3.png?dl=1){width=750px}
...
```{r}
pred.log <- predict(fit.log, newdata = x_test)
pred.dt <- predict(fit.dt, newdata = x_test)
...
confusionMatrix(pred.log, y_test, positive = "Yes")
print("====== Out-of-Sample Validation: Decision Tree ======")
confusionMatrix(pred.dt, y_test, positive = "Yes")
print("====== Out-of-Sample Validation: Random Forest ======")
confusionMatrix(pred.rf, y_test, positive = "Yes")
```
