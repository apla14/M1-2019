...
Since arrays have 2 dimensions, also a 3rd element can be used for slicing&dicinhg.
```{r}
a[1,3,2]
```
...
You can tell R you don't like factors from the start using `stringsAsFactors=FALSE`. I find that annoying. The `tibble` (introduced later) does not do that.
...
For example, we could pipe as follows (donh't worry about the other syntax yet):
```{r, eval = FALSE}
...
Some little exercises in between: Lets go to this [funh exercise on colab](https://colab.research.google.com/drive/1G2pr-SJA5V7QWhMt5fStmlDGU7i2ZUFs), and show what you learned!
